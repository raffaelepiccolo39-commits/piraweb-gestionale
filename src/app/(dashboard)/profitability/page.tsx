...
export default function ProfitabilityPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [allTasksRes, allEntriesRes, allAssignmentsRes] = await Promise.all([
      supabase.from('tasks').select('id, project_id, assigned_to').in('project_id', projectIds.length > 0 ? projectIds : ['']).is('archived_at', null).limit(5000),
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          <Euro size={40} className="mx-auto text-pw-text-dim mb-3" />
