  ...
  return (
    <div className="space-y-6 animate-slide-up">
      ...
      <DataTable
        ...
        searchKeys={[
          (p) => p.name,
          (p) => (p.client as { name?: string; company?: string } | undefined)?.name || '',
          (p) => (p.client as { name?: string; company?: string } | undefined)?.company || '',
          (p) => p.description || '',
        ]}
        searchPlaceholder="Cerca per nome, cliente o descrizione…"
        filters={[
          {
            key: 'status',
