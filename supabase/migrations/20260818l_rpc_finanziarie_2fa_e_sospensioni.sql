-- ============================================================
-- Le RPC finanziarie: rimette il 2FA e insegna loro le sospensioni
-- ============================================================
--
-- Due problemi che si risolvono nello stesso punto.
--
-- 1. LA GUARDIA DEL SECONDO FATTORE NON È MAI ANDATA IN PRODUZIONE.
--    La 20260814g è stata applicata solo in parte: `is_admin_aal2` esiste,
--    le ridefinizioni che venivano dopo no. Si vede dal messaggio d'errore:
--    in produzione le funzioni rispondono 'Accesso non autorizzato', che è
--    la versione della 20260814, invece di 'Accesso non autorizzato: serve
--    la verifica in due passaggi (2FA)'. Restavano protette da is_admin(),
--    quindi non una porta aperta — una serratura in meno del previsto.
--
-- 2. NESSUNA DI LORO SA DELLE MENSILITÀ SOSPESE.
--    La sospensione è arrivata con la 20260818i. Senza filtro un mese
--    sospeso continua a contare come dovuto nel cashflow e nei ricavi per
--    cliente. Oggi non cambia niente perché nessuna rata è sospesa: alla
--    prima che ne sospendi, sì.
--
-- Le due correzioni non stavano insieme in nessun file: la 20260814g ha il
-- 2FA ma non il filtro, la 20260716b il filtro ma non il 2FA. Questa è la
-- 20260814g con i filtri della 20260716b innestati, presi confrontando le
-- due versioni riga per riga invece che a memoria.
--
-- UNA AGGIUNTA NOSTRA, da non confondere con le altre: anche
-- get_profit_loss_summary legge le rate, e nessuna delle due migration le
-- aveva mai messo il filtro. Gliel'ho aggiunto — un mese sospeso non è un
-- ricavo dovuto, ed escluderlo è coerente con le altre quattro.
--
-- ORDINE: dopo la 20260818i, che crea la colonna is_suspended.
-- ============================================================
...
BEGIN
  ...
  WHERE cp.due_date >= p_start_date AND cp.due_date <= p_end_date AND cp.is_suspended = false
